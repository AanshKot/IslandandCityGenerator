...
public class Main{
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(!(config.seed() == null)){
            ...
            island = IslandGenerator.generateIsland(config.shape(), aMesh,config.altitude(),config.lakes(),config.aquifers(),config.soil_type(), config.biome(),"10");
        //Island island = IslandGenerator.generateIsland("square", aMesh,"volcano","4","4");
        }
        
        Structs.Mesh.Builder islandMesh = IslandtoMesh.generateMesh(island,config.map());
